...
const ThemedCheckboxItemWrapper = styled.div`
        ...
        margin-right: 24px;
        cursor: pointer;
        margin-bottom: ${props => props.mode === 'stacked' ? '12px' : null};
    `;
    ...
    const ThemedCheckbox = styled.div`
        ...
        background-color: ${props => props.value ? (props.disabled ? colors.grey.deep : colors.blue.dark) : colors.background};
...
const ThemedCheckboxItem = props => {
    ...
    return (
        <ThemedCheckboxItemWrapper mode={mode} onClick={e => onChange(!value, e)}>
